...
describe("search utility function", () => {
	...
	describe("valid URLs", () => {
		test("returns input when fully qualified URL", () => {
			...
			assert.strictEqual(
				search("http://localhost:8080", template),
				"http://localhost:8080/"
			);
			assert.strictEqual(
				search("https://example.com/path?query=1", template),
	...
	describe("URLs missing protocol", () => {
		test("adds http:// and returns when input has valid hostname with TLD", () => {
			assert.strictEqual(
				search("example.com", template),
				"http://example.com/"
			);
			assert.strictEqual(
				search("sub.example.com/test?q=param", template),
				"http://sub.example.com/test?q=param"
			);
	...
	describe("search queries", () => {
		test("formats search query when input is plain text", () => {
			...
			assert.strictEqual(
				search("test query with spaces", template),
		...
		test("formats search query when input contains special characters", () => {
			assert.strictEqual(
				search("test+query&other=true", template),
				"https://duckduckgo.com/?q=test%2Bquery%26other%3Dtrue"
			);
		});
	});
});
