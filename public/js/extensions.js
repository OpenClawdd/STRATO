...
(function () {
  ...
  const extensionLog = (...args) => {
    if (DEBUG_EXTENSIONS) console.debug(...args);
  };
  const extensionWarn = (...args) => {
    if (DEBUG_EXTENSIONS) console.warn(...args);
  };
  ...
  const DEFAULT_EXTENSIONS = [
    {
      ...
      name: "Ad Blocker",
      version: "1.2.0",
      description: "Block ads in proxied pages for cleaner browsing",
      author: "STRATO Team",
      enabled: true,
      icon: "shield",
      category: "privacy",
    },
    {
      ...
      name: "Chat Bridge",
      version: "1.1.0",
      description: "Connect STRATO Chat with external services",
      author: "STRATO Team",
      enabled: true,
      icon: "message",
      category: "social",
    },
    {
      ...
      name: "Auto Cloak",
      version: "1.0.0",
      description: "Automatically apply stealth when switching tabs",
      author: "STRATO Team",
      enabled: false,
      icon: "eye-off",
      category: "stealth",
    },
    {
      ...
      name: "Quick Notes",
      version: "1.3.0",
      description: "Take notes while browsing, saved locally",
      author: "STRATO Team",
      enabled: false,
      icon: "file-text",
    ...
    },
    {
      ...
      name: "Screenshot Pro",
      version: "1.0.0",
      description: "Capture and annotate screenshots of any page",
      author: "STRATO Team",
      enabled: false,
      icon: "camera",
      category: "tools",
    },
    {
      ...
      name: "Dark Reader",
      version: "1.0.0",
      description: "Force dark mode on all proxied websites",
      author: "STRATO Team",
      enabled: false,
      icon: "moon",
      category: "appearance",
  ...
  function toggleExtension(id) {
    ...
    if (ext) {
      ...
      if (ext.enabled) {
        ...
        if (window.showToast)
          window.showToast(`${ext.name} disabled`, "default");
      }
    }
  }
    ...
    try {
      ...
      const sandbox = {
        ...
        localStorage: {
          ...
          set: (key, value) =>
            localStorage.setItem(`${SANDBOX_KEY}:${ext.id}:${key}`, value),
          remove: (key) =>
            localStorage.removeItem(`${SANDBOX_KEY}:${ext.id}:${key}`),
        },
      ...
      switch (ext.id) {
        ...
        case "chat-bridge":
          sandbox.console.log(
            "Chat bridge active — connecting chat to external services",
          );
          break;
        case "autocloak":
  ...
  function setupQuickNotes(sandbox) {
    ...
    fab.style.cssText =
      "position:fixed;bottom:80px;right:20px;width:40px;height:40px;border-radius:50%;border:1px solid var(--accent-border);background:var(--glass-heavy);backdrop-filter:var(--blur);color:var(--accent);font-size:18px;cursor:pointer;z-index:9999;display:flex;align-items:center;justify-content:center;";
  ...
  function renderExtensions() {
    const grid =
      ...
      document.getElementById("extension-grid");
    if (!grid) return;
  ...
  function renderGallery(extensions) {
    const container =
      document.getElementById("extension-gallery-grid") ||
      document.getElementById("extension-gallery") ||
  ...
  async function installExtension(ext) {
    ...
    if (extensions.find((e) => e.id === ext.id)) {
      if (window.showToast)
        window.showToast("Extension already installed", "default");
      return;
    }
