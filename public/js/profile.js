...
(function () {
  ...
  function addXP(amount) {
    ...
    if (newLevel.level > oldLevel.level) {
      ...
      if (window.STRATO_NOTIFY)
        window.STRATO_NOTIFY(`Level Up! Level ${newLevel.level}`, "levelup");
    }
  ...
  function updateXpUI() {
    ...
    if (homeXpText)
      homeXpText.textContent = `${info.currentXp} / ${info.requiredXp} XP`;
    ...
    if (profileXpText)
      profileXpText.textContent = `${info.currentXp} / ${info.requiredXp} XP`;
    ...
    if (statGamesPlayed)
      statGamesPlayed.textContent =
        localStorage.getItem("strato-gamesPlayed") || "0";
    if (statPagesBrowsed)
      statPagesBrowsed.textContent =
        localStorage.getItem("strato-pagesLoaded") || "0";
    if (statAiChats)
      statAiChats.textContent =
        localStorage.getItem("strato-aiMessagesSent") || "0";
    if (statChatMsgs)
      statChatMsgs.textContent =
        localStorage.getItem("strato-chatMessages") || "0";
  }
  ...
  async function updateProfile(updates) {
    ...
    try {
      ...
      if (resp.ok) {
        if (updates.username)
          localStorage.setItem("strato-username", updates.username);
  ...
  async function submitScore(gameId, score) {
    const localScores = JSON.parse(
      localStorage.getItem("strato-scores") || "{}",
    );
  ...
  async function loadLeaderboard(period) {
    try {
      const resp = await fetch(
        `/api/leaderboard${period ? `?period=${period}` : ""}`,
      );
  ...
  function getLocalLeaderboard() {
    ...
    return [
      { rank: 1, name: profile.username, score: profile.xp, badge: "gold" },
      {
        rank: 2,
        name: "CyberNinja",
        score: Math.floor(profile.xp * 0.7),
        badge: "silver",
      },
      {
        rank: 3,
        name: "PixelMaster",
        score: Math.floor(profile.xp * 0.5),
        badge: "bronze",
      },
      {
        rank: 4,
        name: "GhostRider",
        score: Math.floor(profile.xp * 0.35),
        badge: "",
      },
      {
        rank: 5,
        name: "StarCoder",
        score: Math.floor(profile.xp * 0.2),
        badge: "",
      },
    ];
  }
  ...
  function renderLeaderboard(leaders) {
    const list =
      document.getElementById("settings-leaderboard") ||
    ...
    if (!leaders || leaders.length === 0) {
      list.innerHTML =
        '<div style="color:var(--fg-faint);text-align:center;padding:16px">No entries yet</div>';
      return;
    }
  ...
  function init() {
    ...
    document.querySelectorAll("[data-leaderboard-period]").forEach((btn) => {
      btn.addEventListener("click", () => {
        document
          .querySelectorAll("[data-leaderboard-period]")
