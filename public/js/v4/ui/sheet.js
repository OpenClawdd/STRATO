import { categoryOf, descriptionOf, findGame, nameOf, similarGames, tagsOf } from '../core/catalog.js';
  ...
  overlay.innerHTML = `<div class="game-sheet" role="dialog" aria-modal="true" aria-labelledby="sheet-title">
    ...
    <div class="game-sheet-grid">
      ...
      <div class="sheet-copy">
        <p class="section-eyebrow">Pre-launch</p>
        ...
        <div class="hideout-tags">${tagsOf(game).slice(0, 5).map((tag) => `<span>${escapeHtml(tag)}</span>`).join('')}</div>
        ...
        ${similar.length ? `<div class="nearby-list"><p class="home-result-meta">Similar games</p>${similar.map((item) => `<button class="similar-game-btn" data-open-id="${escapeHtml(item.id)}" type="button"><span>${escapeHtml(nameOf(item))}</span><span>${escapeHtml(categoryOf(item))}</span></button>`).join('')}</div>` : ''}
      </div>
    </div>
  </div>`;
