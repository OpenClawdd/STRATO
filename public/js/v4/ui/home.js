...
import { categoryOf, findGame, moodClusters, nameOf, playableCatalog, tagsOf, visibleCatalog } from '../core/catalog.js';
...
import { card, escapeHtml, fallbackThumb, statusLabel, thumb } from './cards.js';
...
function renderCards(id, list, emptyText, controller, variant = '') {
  ...
  if (!list.length) {
    ...
    container.querySelector('[data-focus-search]')?.addEventListener('click', () => controller.focusSearch());
    return;
  }
...
function renderPulse() {
  ...
  pulse.innerHTML = [
    ...
    ['Signal lost', stats['recently-failed'] || 0],
  ].map(([label, value]) => `<div class="pulse-tile"><span>${escapeHtml(label)}</span><strong>${value}</strong></div>`).join('');
}
...
function renderMoods(controller) {
  ...
  const buttons = [`<button class="mood-chip ${state.activeMood === 'all' ? 'active' : ''}" data-mood="all" type="button">All</button>`]
    .concat(clusters.map(({ name, count }) => `<button class="mood-chip ${state.activeMood === name ? 'active' : ''}" data-mood="${escapeHtml(name)}" type="button">${escapeHtml(name)} <span>${count}</span></button>`))
    .join('');
  if (container) container.innerHTML = buttons;
  if (quick) quick.innerHTML = clusters.slice(0, 5).map(({ name }) => `<button class="mood-chip ${state.activeMood === name ? 'active' : ''}" data-mood="${escapeHtml(name)}" type="button">${escapeHtml(name)}</button>`).join('');
...
export function createHomeController() {
  const controller = {
    render() {
      ...
      const favorites = readJson(keys.favorites, []).map(findGame).filter((game) => game && list.includes(game)).slice(0, 6);
      const recent = readJson(keys.recent, []).map(findGame).filter((game) => game && list.includes(game)).slice(0, 6);
      const counts = readJson(keys.playCounts, {});
      const most = Object.entries(counts)
        .map(([id, count]) => ({ game: findGame(id), count: Number(count) || 0 }))
      ...
      renderCards('daily-picks', dailyPicks(), 'No daily picks are launchable yet.', controller, 'featured');
      renderCards('home-favorites', favorites, 'Your favorites are empty. Pin something you want to return to.', controller);
      renderCards('home-recent', recent, 'No recent launches yet. Launch something.', controller);
    ...
    search(query) {
      ...
      if (!results.length) {
        ...
        container.querySelector('#empty-surprise')?.addEventListener('click', () => controller.surprise());
        return;
      }
    ...
    async launch(id) {
      await launchById(id, {
        onFail: (game, reason) => showRecovery(game, reason, {
    ...
    toggleFavorite(id) {
      ...
      toast(index >= 0 ? 'Removed from favorites.' : 'Pinned to favorites.');
      controller.render();
    },
    ...
    surprise() {
      ...
      if (!game) {
        toast('No launchable games available.');
        return;
      }
      const button = document.getElementById('surprise-me');
      if (button && !preferences().lowPower && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
