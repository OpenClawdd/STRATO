...
export function health(game) {
  if (!game || !game.id)
    return { status: "invalid", reason: "Unavailable entry" };
  if (!game.url) return { status: "missing-url", reason: "Missing URL" };
  if (game.needsConfig || game.config_required || isPlaceholder(game.url))
    return { status: "needs-config", reason: "Needs config" };
  if (!isValidUrl(game))
    return { status: "invalid", reason: "Unsupported URL" };
  ...
  if (!game.thumbnail || isPlaceholder(game.thumbnail))
    return { status: "fallback-art", reason: "Using fallback art" };
