...
export function applyPreferences() {
  ...
  document.getElementById('low-power-toggle')?.setAttribute('aria-pressed', String(Boolean(prefs.lowPower)));
  document.getElementById('compact-toggle')?.setAttribute('aria-pressed', String(Boolean(prefs.compact)));
}
...
export function bindSettings({ onUpdate } = {}) {
  ...
  document.getElementById('reset-local-data')?.addEventListener('click', () => {
    removeKeys([keys.favorites, keys.recent, keys.playCounts, keys.lastPlayed, keys.failures, keys.preferences]);
    applyPreferences();
