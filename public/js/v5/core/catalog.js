...
import { blockedCategories, blockedTerms, isLaunchable, launchability } from './health.js';
...
export function normalizeGame(game) {
  ...
  return {
    ...
    tags,
    description,
    searchableText: [title, category, description, ...tags].join(' ').toLowerCase(),
...
export function allNormalized() {
  return state.normalized.length ? state.normalized : state.games.map(normalizeGame);
}

export function playableCatalog() {
  return allNormalized().filter((game) => isHomeSafe(game) && isLaunchable(game) && game.reliability !== 'red');
}
