...
function renderFirstRunHint(home) {
  ...
  hint.innerHTML = '<span>Search anything. Launch instantly.</span><button type="button">Got it</button>';
...
function renderLaunchBay() {
  ...
  const copy = state.launchBay.status === 'loading'
    ? `Loading ${title}…`
...
function bindLaunchBay() {
  ...
  const sync = () => { body?.classList.toggle('has-launch', Boolean(iframe?.src && iframe.src !== window.location.href)); body?.classList.remove('is-loading'); renderLaunchBay(); };
