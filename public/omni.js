...
/* ═══════════════════
   FPS VITALS
   ═══════════════════ */
let _lastTime = performance.now(), _frames = 0;
function runVitals() {
...
function proxifyUrl(url) {
  ...
  if (engine === "splash" && window.__scramjet$config)  return __scramjet$config.prefix + __scramjet$config.codec.encode(url);
...
function switchView(viewId) {
  ...
  document.querySelectorAll(".sn-item").forEach(i => i.classList.toggle("on", i.dataset.view === viewId));
...
function createTile(game, index = 0) {
  ...
  const launch = () => { if (url) openOverlay(url, title); };
...
function renderGrid(list) {
  ...
  list.slice(0, currentLimit).forEach((g, i) => frag.appendChild(createTile(g, i)));
...
function appendGrid(list, limit) {
  ...
  const frag = document.createDocumentFragment();
  list.slice(start, limit).forEach((g, i) => frag.appendChild(createTile(g, start + i)));
  grid.appendChild(frag);
}
...
function applyFilters() {
  ...
  let result = base;
  if (activeCategory !== "All") result = result.filter(g => (g.t || "Other") === activeCategory);
  if (searchQuery)               result = result.filter(g => (g.title || g.n || "").toLowerCase().includes(searchQuery));

  filteredGames = result;
  currentLimit  = RENDER_LIMIT;

  const inp = $("searchInput");
  if (inp) inp.placeholder = `Search ${base.length.toLocaleString()} ${isDecoy ? "academic" : "games"}...`;
...
function buildCategoryBar() {
  ...
  const counts = {};
  base.forEach(g => { const c = g.t || "Other"; counts[c] = (counts[c] || 0) + 1; });
...
async function fetchExternalGames() {
  ...
  for (const src of sources) {
    try {
      ...
      if (added > 0) { buildCategoryBar(); applyFilters(); }
    } catch {}
  }
}
...
function initSearch() {
  ...
  input.addEventListener("input", e => {
    ...
    timer = setTimeout(() => {
      ...
      applyFilters();
      if (!$("view-grid")?.classList.contains("active"))
        switchView("grid");
    }, DEBOUNCE_MS);
  });
...
function initMediaCards() {
  document.querySelectorAll("[data-action='watch'], [data-action='listen']").forEach(el => {
    el.addEventListener("click", () => {
      openOverlay(el.dataset.url, el.dataset.name || el.querySelector(".mc-title")?.textContent);
    });
  });
}
...
function initAI() {
  ...
  const RESPONSES = {
    math:    ["Let's evaluate the derivative across the x-axis.", "Consider the quadratic formula: x = (−b ± √(b²−4ac)) / 2a.", "We can factor this to find the roots.", "The slope of this function is determined by its first derivative."],
    history: ["This event shaped the geopolitical landscape of the era.", "Scholars argue economic factors were the primary cause.", "The resulting treaty established new diplomatic precedents.", "Primary sources from this period suggest a different narrative."],
    science: ["Note how the molecular bonds react under thermal pressure.", "This demonstrates the first law of thermodynamics.", "The cell undergoes mitosis through four distinct phases.", "Quantum superposition allows the particle to exist in both states."],
    default: ["That's a great question! Let me break it down step by step.", "Based on your question, here is the key concept to understand.", "The answer depends on a few variables — let's examine each one."],
  };
  ...
  function send() {
    ...
    setTimeout(() => {
      ...
      let cat = "default";
      if (/math|calc|algebra|equat|formula|geom|integral|deriv/.test(lower)) cat = "math";
      else if (/histor|war|treaty|civil|century|politics/.test(lower)) cat = "history";
      else if (/scienc|chem|bio|physic|cell|molecule|energy|force/.test(lower)) cat = "science";

      const pool = RESPONSES[cat];
      const resp = pool[Math.floor(Math.random() * pool.length)];
      const extra = RESPONSES.default[Math.floor(Math.random() * RESPONSES.default.length)];
...
function initSettings() {
  ...
  // Load saved values
  if (panicInput) panicInput.value = localStorage.getItem("strato_panic") || "https://classroom.google.com";
  ...
  if (proxyEngBrowser) {
    ...
    proxyEngBrowser.addEventListener("change", e => localStorage.setItem("strato_proxy", e.target.value));
  }
  ...
  if (cloakOn && cloakOff) {
    ...
    cloakOn.addEventListener("click",  () => { localStorage.setItem("strato_cloak", "true");  cloakOn.classList.add("active"); cloakOff.classList.remove("active"); });
    cloakOff.addEventListener("click", () => { localStorage.setItem("strato_cloak", "false"); cloakOff.classList.add("active"); cloakOn.classList.remove("active"); });
  }
  ...
  if (mathOn && mathOff) {
    ...
    mathOn.addEventListener("click",  () => { localStorage.setItem("strato_math_decoy", "true");  mathOn.classList.add("active"); mathOff.classList.remove("active"); buildCategoryBar(); applyFilters(); });
    mathOff.addEventListener("click", () => { localStorage.setItem("strato_math_decoy", "false"); mathOff.classList.add("active"); mathOn.classList.remove("active"); buildCategoryBar(); applyFilters(); });
  }
  ...
  if (clearBtn) {
    clearBtn.addEventListener("click", () => {
      if (confirm("Clear all settings and cached data?")) { localStorage.clear(); location.reload(); }
    });
  }
}
...
function initCloak() {
  const PRESETS = {
    default:   { title: "STRATO",          fav: "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><text y='26' font-size='26'>◆</text></svg>" },
    drive:     { title: "My Drive — Google Drive",   fav: "https://ssl.gstatic.com/images/branding/product/1x/drive_2020q4_32dp.png" },
    classroom: { title: "Stream — Google Classroom", fav: "https://ssl.gstatic.com/classroom/favicon.png" },
  };
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    if (document.hidden) {
      ...
      if (fav) fav.href = "https://ssl.gstatic.com/images/branding/product/1x/drive_2020q4_32dp.png";
    } else {
      document.title = "STRATO";
      if (fav) fav.href = "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><text y='26' font-size='26'>◆</text></svg>";
    }
  });
...
function panic() {
  location.replace(localStorage.getItem("strato_panic") || "https://classroom.google.com");
}
...
function igniteStratosphere(targetView = "grid") {
  ...
  setTimeout(() => {
    ...
    const messages = ["CONNECTING TO WISP RELAY...", "MOUNTING PROXY LAYERS...", "LOADING GAME ARRAY...", "STRATOSPHERE READY"];
    let progress = 0;
    let msgIdx = 0;

    const tick = setInterval(() => {
      progress += progress < 75 ? Math.random() * 14 : Math.random() * 3;
      if (progress >= 100) { progress = 100; clearInterval(tick); finalize(); }
      bar.style.width = progress + "%";
      const newIdx = Math.floor((progress / 100) * messages.length);
      if (newIdx !== msgIdx && newIdx < messages.length) { msgIdx = newIdx; if (status) status.textContent = messages[msgIdx]; }
    }, 90);
...
function initNav() {
  ...
  $("overlay-fs-btn")?.addEventListener("click", () => {
    const ifr = $("game-iframe");
    (ifr?.requestFullscreen || ifr?.webkitRequestFullscreen || (() => {})).call(ifr);
  });
...
async function boot() {
  ...
  try {
    ...
    const spamFilter = /badge|listed|hunt|stash|vault|directory|powered|dang\.ai|launchlist/i;
    ...
    const inp = $("searchInput");
    if (inp) inp.placeholder = `Search ${masterGames.length.toLocaleString()} games...`;
