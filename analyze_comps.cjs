...
async function scan() {
	...
	await Promise.all(
		urls.map(async (url) => {
			try {
				...
				const hasUv =
					html.includes("uv.bundle.js") || html.includes("__uv$config");
				const hasScramjet =
					html.includes("scramjet.worker.js") || html.includes("__scramjet");
				const hasRammerhead = html.includes("rammerhead.js");
				const hasCloak =
					html.includes("cloak") ||
					html.includes("about:blank") ||
					html.includes("panickey");

				// Look for frameworks
				const hasTailwind = html.includes("tailwind");
				const hasReact =
					html.includes("data-reactroot") || html.includes("react");
				const hasVite =
					html.includes("vite.config.js") || html.includes("data-v-");
	...
	fs.writeFileSync(
		"c:/Users/noahm/Downloads/STRATO/comp_analysis.json",
