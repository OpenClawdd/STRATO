...
const adfreePath = "/Users/noahmendieta/.gemini/antigravity-cli/brain/4b2ecb56-566e-4a3c-b919-f74eacec3cac/scratch/adfree-games.json";
const tkhoPath = "/Users/noahmendieta/.gemini/antigravity-cli/brain/4b2ecb56-566e-4a3c-b919-f74eacec3cac/scratch/3kh0-games.json";

const games = JSON.parse(fs.readFileSync(catalogPath, "utf8"));
const adfree = fs.existsSync(adfreePath) ? JSON.parse(fs.readFileSync(adfreePath, "utf8")) : [];
const tkho = fs.existsSync(tkhoPath) ? JSON.parse(fs.readFileSync(tkhoPath, "utf8")) : [];
...
const updatedGames = games.map(game => {
  ...
  if (adfreeMap.has(slug)) {
    ...
    repairedCount++;
    return { ...game, url: cleanUrl, reliability: "yellow", source: "adfree-resurrection" };
  }
  ...
  if (tkhoMap.has(slug)) {
    ...
    repairedCount++;
    return { ...game, url: cleanUrl, reliability: "yellow", source: "3kh0-resurrection" };
  }
