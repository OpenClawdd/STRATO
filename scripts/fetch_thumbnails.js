...
async function main() {
  ...
  const gameRegex = /{\s*n:\s*"([^"]+)",\s*e:\s*"([^"]+)",\s*u:\s*"([^"]+)"\s*}/g;
  ...
  for (const game of games) {
    ...
    try {
      ...
      if (images && images.length > 0) {
        ...
        updatedGamesString = updatedGamesString.replace(game.originalString, newGameString);
  ...
  const oldInnerHTML = '<div class="card-icon">${g.e}</div><div class="card-label">${g.n}</div>';
  const newInnerHTML = '<img src="${g.img}" class="card-img" alt="${g.n}"><div class="card-label">${g.n}</div>';

  if (htmlContent.includes(oldInnerHTML)) {
      htmlContent = htmlContent.replace(oldInnerHTML, newInnerHTML);
  } else {
      console.warn("Could not find innerHTML string to replace in buildGames function.");
  }
