...
router.post('/api/bookmarks', async (req, res) => {
  try {
    ...
    if (favicon && (typeof favicon !== 'string' || favicon.length > 500)) {
      return res.status(400).json({ error: 'Favicon must be under 500 characters' });
    }
    ...
    if (existing) {
      return res.status(409).json({ error: 'Bookmark already exists', id: existing.id });
    }
...
router.delete('/api/history', async (req, res) => {
  try {
    ...
    const removed = await store.deleteMany('history', (h) => h.username === username);
