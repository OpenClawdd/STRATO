...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Defer `.map()` operation until after `.sort()` and `.slice()`
    // to reduce memory overhead and avoid O(N) short-lived garbage object allocations.
    // Also create a shallow copy before sorting to avoid mutating cached data.
