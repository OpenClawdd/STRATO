...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Use getTopN to avoid full array sort (O(N) vs O(N log N))
...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Get top N first, then map only the winners. Prevents
    // iterating map over the full user db and avoids full array sort.
    const topN = getTopN(allUsers, 25, (u) => u.xp || 0);
    const sorted = topN.map((u) => ({
      username: u.username,
      xp: u.xp || 0,
      level: u.level || 1,
      coins: u.coins || 0,
