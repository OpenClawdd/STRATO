...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    // Optimization: Compare ISO date strings directly rather than instantiating Date objects in O(N) loops
    if (period === "daily") {
      const dayAgoStr = new Date(
        Date.now() - 24 * 60 * 60 * 1000,
      ).toISOString();
      scores = scores.filter((s) => (s.created_at || "") > dayAgoStr);
    } else if (period === "weekly") {
      const weekAgoStr = new Date(
        Date.now() - 7 * 24 * 60 * 60 * 1000,
      ).toISOString();
      scores = scores.filter((s) => (s.created_at || "") > weekAgoStr);
    }
