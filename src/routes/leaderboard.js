...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    // O(N) bounded insertion sort to find top 10 without massive array allocations
    const top10 = getTopNDescending(
      allScores,
      10,
      (s) => s.score,
      (s) =>
        s.gameId === gameId &&
        (minTime === 0 || new Date(s.created_at).getTime() > minTime),
    );
...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // O(N) bounded insertion sort to find top 25 users by XP
    const top25 = getTopNDescending(allUsers, 25, (u) => u.xp);
