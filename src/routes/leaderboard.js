...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    let timeFilter = 0;
    if (period === "daily") {
      timeFilter = Date.now() - 24 * 60 * 60 * 1000;
    } else if (period === "weekly") {
      timeFilter = Date.now() - 7 * 24 * 60 * 60 * 1000;
    }

    // Use bounded insertion sort (O(N)) to find top 10 scores
    // Filters by gameId and time period during the pass
    const top10 = getTopN(
      allScores,
      10,
      (s) => s.score,
      (s) => {
        ...
        if (
          timeFilter > 0 &&
          !(new Date(s.created_at).getTime() > timeFilter)
        ) {
          return false;
        }
        return true;
...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // Map only the final 25 elements instead of the whole array
