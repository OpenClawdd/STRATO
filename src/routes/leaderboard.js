...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    // Sort by score descending, take top 10 using O(N) bounded insertion sort
...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // Sort by XP descending using O(N) bounded insertion sort, mapping only top 25 users
    const sorted = getTopN(allUsers, 25, (u) => u.xp).map((u) => ({
      username: u.username,
      xp: u.xp || 0,
      level: u.level || 1,
      coins: u.coins || 0,
