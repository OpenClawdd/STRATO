...
import { getTopN } from "../db/sort.js";
...
router.get("/api/leaderboard/:gameId", async (req, res) => {
  try {
    ...
    // Determine minimum timestamp based on period
    let minTimestamp = 0;
    if (period === "daily") {
    ...
    // Get top 10 scores using bounded insertion sort (O(N) instead of O(N log N))
    const top10 = getTopN(
      allScores,
      10,
      (a, b) => b.score - a.score, // Sort descending
      (s) =>
        s.gameId === gameId &&
        (minTimestamp === 0 || new Date(s.created_at).getTime() > minTimestamp),
    );
...
router.get("/api/leaderboard", async (req, res) => {
  try {
    ...
    // Get top 25 users by XP using bounded insertion sort (O(N) instead of O(N log N))
    const top25 = getTopN(
      allUsers,
      25,
      (a, b) => (b.xp || 0) - (a.xp || 0), // Sort descending by XP
    );
