...
router.post('/api/themes', async (req, res) => {
  try {
    ...
    if (!code || typeof code !== 'string' || !/^[a-z0-9-]+$/.test(code) || code.length > 50) {
      return res.status(400).json({ error: 'Theme code must be lowercase alphanumeric with dashes, max 50 chars' });
    }
...
router.delete('/api/themes/:code', async (req, res) => {
  try {
    ...
    if (theme.created_by !== username) {
      return res.status(403).json({ error: 'Only the creator can delete this theme' });
    }
