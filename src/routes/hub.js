...
router.get("/api/mirrors/status", (req, res) => {
  ...
  res.json({
    ...
    mirrors: (data.mirrors || []).map((m) => ({
      ...
      ...(m.alternates?.some((a) => !isUnresolved(a))
        ? { alternates: m.alternates.filter((a) => !isUnresolved(a)) }
        : {}),
    })),
  });
});
...
router.get("/api/proxy/health", async (req, res) => {
  ...
  try {
    const uvResp = await fetch(
      `http://localhost:${DEFAULT_PORT}/frog/uv.config.js`,
      {
        method: "GET",
  ...
  try {
    const sjResp = await fetch(
      `http://localhost:${DEFAULT_PORT}/scramjet/config.js`,
      {
        method: "GET",
        signal: AbortSignal.timeout(3000),
      },
    ...
    try {
      const sjResp2 = await fetch(
        `http://localhost:${DEFAULT_PORT}/scramjet/sw.js`,
        {
          method: "GET",
          signal: AbortSignal.timeout(3000),
        },
