...
// ── Login page route (served by auth middleware with CSRF token) ──
...
router.post('/login', (req, res) => {
  // Handled entirely by auth middleware — this route just needs to exist
  // The middleware will redirect on success or send error page on failure
});
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>STRATO — Login</title>
  <meta name="csrf-token" content="${csrfToken}">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=Manrope:wght@400;500;600&family=JetBrains+Mono:wght@400;500&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <div id="login-view" class="login-container">
    <div class="glass login-card">
      <h1 class="login-logo">STRATO</h1>
      <p class="login-subtitle">Above the clouds. Above restrictions.</p>
      <form method="POST" action="/login" class="login-form">
        ...
        <div class="form-group tos-group">
          <label class="tos-label">
            <input type="checkbox" id="tos-checkbox" name="tos_accepted" value="true" required>
            <span>I agree to the Terms of Service. I will use STRATO responsibly.</span>
          </label>
        </div>
        <button type="submit" class="glass-btn primary login-btn">Enter STRATO</button>
      </form>
    </div>
  </div>
</body>
