...
router.get("/api/analytics/global", async (req, res) => {
  try {
    ...
    const topChatEntries = getTopN(Object.entries(chatCount), 10, (e) => e[1]);
    const topChatters = topChatEntries.map(([username, count]) => ({
      username,
      messages: count,
    }));
    ...
    const topGameEntries = getTopN(Object.entries(gameCount), 10, (e) => e[1]);
    const popularGames = topGameEntries.map(([game, count]) => ({
      game,
      plays: count,
    }));
