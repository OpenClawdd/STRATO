...
router.get("/api/analytics/global", async (req, res) => {
  try {
    ...
    // Top players by XP
    const topByXpRaw = getTopN(users, 20, (u) => u.xp || 0);
    const topByXp = topByXpRaw.map((u) => ({
      username: u.username,
      avatar: u.avatar,
      xp: u.xp || 0,
      level: u.level || 1,
    ...
    const topChattersRaw = getTopN(Object.entries(chatCount), 10, (e) => e[1]);
    const topChatters = topChattersRaw.map(([username, count]) => ({
      username,
      messages: count,
    }));
    ...
    const popularGamesRaw = getTopN(Object.entries(gameCount), 10, (e) => e[1]);
    const popularGames = popularGamesRaw.map(([game, count]) => ({
      game,
      plays: count,
    }));
    ...
    if (user) {
      ...
      switch (sanitizedCategory) {
        ...
        case "ai":
          ...
          break;
        case "bookmark":
          updates.stats.bookmarks_count =
            (updates.stats.bookmarks_count || 0) + 1;
          updates.xp = (user.xp || 0) + 2;
          break;
      }
