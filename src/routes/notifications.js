...
router.get("/api/analytics/personal", async (req, res) => {
  try {
    ...
    res.json({
      ...
      recentScores: topNElements(userScores, 5, (s) =>
...
router.get("/api/analytics/global", async (req, res) => {
  try {
    ...
    // Top players by XP
    const topByXp = topNElements(users, 20, (u) => u.xp || 0).map((u) => ({
      username: u.username,
      avatar: u.avatar,
      xp: u.xp || 0,
      level: u.level || 1,
    ...
    const topChatters = topNElements(
    ...
    const popularGames = topNElements(
