...
router.get("/api/analytics/global", async (req, res) => {
  try {
    ...
    // Top players by XP
    const topUsers = getTopN(users, 20, (u) => u.xp);
    ...
    const chatEntries = Object.entries(chatCount);
    const topChatters = getTopN(chatEntries, 10, (e) => e[1]).map(
    ...
    const gameEntries = Object.entries(gameCount);
    const popularGames = getTopN(gameEntries, 10, (e) => e[1]).map(
      ([game, count]) => ({ game, plays: count }),
    );
