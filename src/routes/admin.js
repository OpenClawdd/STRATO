...
import { topNElements } from "../utils/sort.js";
...
router.delete("/api/admin/users/:id", async (req, res) => {
  try {
    ...
    await store.deleteMany(
      "scores",
      (s) => s.id === id || s.userId === id || s.username === username,
    );
    await store.deleteMany(
      "bookmarks",
      (b) => b.userId === id || b.username === username,
    );
    await store.deleteMany(
      "saves",
      (s) => s.userId === id || s.username === username,
    );
...
router.get("/api/admin/analytics", async (req, res) => {
  try {
    ...
    // Top users by XP
    const topUsers = topNElements(users, 10, (u) => u.xp || 0).map((u) => ({
      username: u.username,
      xp: u.xp || 0,
      level: u.level || 1,
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      chatActivity[dateStr] = chatMessages.filter(
    ...
    res.json({
      ...
      topUsers,
      chatActivity,
      gamesLeaderboard: topNElements(scores, 10, (s) => s.score || 0).map(
        (s) => ({
          username: s.username,
          game: s.game,
          score: s.score,
...
router.post("/api/admin/cleanup", async (req, res) => {
  try {
    ...
    const chatDeleted = await store.deleteMany(
      "chat_messages",
