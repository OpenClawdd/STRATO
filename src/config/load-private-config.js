...
function resolveValue(value, privateConfig = {}) {
  ...
  if (Array.isArray(value))
    return value.map((v) => resolveValue(v, privateConfig));
...
export function resolveConfig(relativePath) {
  ...
  try {
    ...
    console.error(
      `[STRATO] Failed to load config ${relativePath}:`,
      err.message,
    );
  ...
  function findUnresolved(obj, path = "") {
    ...
    if (Array.isArray(obj))
      obj.forEach((v, i) => findUnresolved(v, `${path}[${i}]`));
    if (obj && typeof obj === "object" && !Array.isArray(obj)) {
      for (const [k, v] of Object.entries(obj))
        findUnresolved(v, `${path}.${k}`);
    }
  }
  ...
  console.log(
    `[STRATO] Config ${relativePath}: ${unresolved.length} unresolved, ${totalPlaceholders} from private config`,
  );

  return {
    data: resolved,
    unresolved,
    resolved: totalPlaceholders - unresolved.length,
  };
}
